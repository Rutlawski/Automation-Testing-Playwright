...
test("Adds a product, incorrectly fills payment data empty and verifies", async ({page}) => {
    ...
    await deliveryDetailsPage.fillUserPaymentDetailsIncorrect();
    await deliveryDetailsPage.clickContinueToPaymentButton();
    const errorMessage = await deliveryDetailsPage.errorMessage.innerText();
    await expect(errorMessage).toBe("Error: First name can't contain numbers");
})